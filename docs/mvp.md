...
DECIDE V1 supports reviewed rivalry, landmark, setting and classic-ground facts plus **Exceptional support** for a home club whose completed-season attendance evidence is materially exceptional for its competition. Attendance evidence informs editorial publication; it is not calculated as a live recommendation score. A fact may be marked `LEAD` or `NORMAL` so the strongest reviewed reason can lead regardless of category. When priorities are equal, rivalry remains the fixture-specific fallback before the stable category and label order. Discover shows one lead reason; fixture detail may show all applicable published reasons.
