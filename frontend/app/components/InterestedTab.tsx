...
function MatchdayCard({ fixture, venueName, venueCity, state, footer }: { fixture: CardFixture; venueName: string | null; venueCity: string | null; state: "upcoming" | "answer" | "attended"; footer: ReactNode }) {
  ...
  return <article className={`min-w-0 border-b-2 border-[var(--tt-ink)] py-4 ${state === "answer" ? "border-l-4 border-l-[var(--brand-interactive)] pl-3" : ""}`}>
    <p className="text-xs font-bold uppercase tracking-[0.08em] text-[var(--tt-muted)]">{displayDate(fixture.fixture_date, state !== "attended")}{fixture.league_name ? ` · ${fixture.league_name}` : ""}</p>
    <Link href={`/fixture/${fixture.fixture_id}`} className="mt-2 grid min-w-0 grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-3 hover:text-[var(--brand-interactive)]">
    ...
    <p className="mt-2 min-w-0 break-words text-sm text-[var(--tt-muted)]">{venueName ?? "Ground to be confirmed"}{venueCity ? ` · ${venueCity}` : ""}</p><footer className="mt-2">{footer}</footer>
  </article>;
}
