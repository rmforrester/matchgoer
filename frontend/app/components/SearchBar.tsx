  ...
  return (
    <div className="min-w-0">
      ...
      {open && <div className="fixed inset-0 z-[2000] flex items-end bg-black/40 p-2 sm:items-center sm:justify-center" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}>
        <section role="dialog" aria-modal="true" aria-label="Choose match dates" className="max-h-[calc(100dvh-1rem)] w-full overflow-y-auto border-2 border-[var(--tt-ink)] bg-[var(--tt-paper)] p-3 shadow-[5px_5px_0_var(--tt-blue)] sm:max-w-md sm:p-4">
          ...
          <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">{([['Today','today'],['Tomorrow','tomorrow'],['This weekend','weekend'],['Next weekend','next-weekend']] as const).map(([label, kind]) => <button key={kind} type="button" onClick={() => shortcut(kind)} className="min-h-11 border border-[var(--tt-ink)] px-2 text-xs font-extrabold uppercase hover:bg-[var(--tt-blue)] hover:text-white">{label}</button>)}</div>
