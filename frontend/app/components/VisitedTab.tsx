...
export default function VisitedTab() {
  ...
  return <main className="mx-auto w-full max-w-5xl px-4 py-4 sm:px-6 sm:py-8">
    ...
    {!loading && grounds.length > 0 && <section className="mt-4" aria-labelledby="football-map-heading"><h2 id="football-map-heading" className="sr-only">My football world map</h2><p className="mb-3 text-xs font-extrabold uppercase tracking-[0.09em] text-[var(--brand-interactive)]">{grounds.length} grounds{cityCount > 0 ? ` · ${cityCount} cities` : ""}{countryCount > 0 ? ` · ${countryCount} countries` : ""} · {matchdayCount} matchdays</p><PersonalGroundMap grounds={grounds}/>{unplottableCount > 0 && <p className="mt-2 text-xs font-bold text-[var(--tt-muted)]">{unplottableCount} visited {unplottableCount === 1 ? "ground could" : "grounds could"} not be plotted because coordinates are unavailable.</p>}</section>}
