...
export default function MatchdayTips({ tips = [], venueId, inline = false }: Props) {
  ...
  if (tipList.length === 0) return <section id="tips" className="mt-6 scroll-mt-6" aria-labelledby="tips-heading"><span id="tips-add" className="block scroll-mt-6" aria-hidden="true"/><h2 id="tips-heading" className="sr-only">Matchday tips</h2><p className="text-sm font-bold">Know this ground? <button type="button" onClick={toggleForm} className="min-h-11 text-[var(--tt-blue)] underline decoration-2 underline-offset-4">Add a tip →</button></p>{tipForm}{error && <p role="alert" className="mt-2 text-sm font-semibold text-red-800">{error}</p>}</section>;
