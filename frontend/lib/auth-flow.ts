...
type AnonymousSessionState = { anonymous: boolean; anonymous_activity?: boolean };
type ConversionHandoff = { handoff_token: string; expires_at: string };

export async function prepareSigninConversion(
  returnTo: string,
  dependencies: {
    readSession: () => Promise<AnonymousSessionState>;
    issueHandoff: () => Promise<ConversionHandoff>;
    saveHandoff: (value: { token: string; expiresAt: string; returnTo: string }) => void;
  },
) {
  const session = await dependencies.readSession();
  if (!session.anonymous || !session.anonymous_activity) return null;
  const handoff = await dependencies.issueHandoff();
  dependencies.saveHandoff({
    token: handoff.handoff_token,
    expiresAt: handoff.expires_at,
    returnTo,
  });
  return handoff.handoff_token;
}
