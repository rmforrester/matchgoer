...
REVIEWED_VENUE_ALIASES = {("Italy", "San Siro"): 23100}
...
def load_identity_inventory(db) -> dict:
    ...
    for (country, alias), venue_id in REVIEWED_VENUE_ALIASES.items():
        if venue_id in represented_venue_ids:
            venue_index.setdefault((country, normalize_identity(alias)), set()).add(venue_id)
    return {
        "teams": teams,
        "venues": venues,
        "published": published,
...
def reconcile_row(row: dict, inventory: dict) -> dict:
    ...
    if scope == "TEAM_PAIR":
        ...
        if all(len(candidate) == 1 for candidate in candidates):
            ...
            plausible = [_plausible(part, country, inventory["teams"], "team_name") for part in parts]
            status = "NEEDS_IDENTITY_REVIEW" if any(plausible) else "NOT_IN_CURRENT_INVENTORY"
            return _result(row, [], status, f"No exact canonical pair; plausible candidates: {plausible}" if any(plausible) else "One or both teams are absent from current fixtures")
    elif scope == "TEAM":
